...
__global__ void reduceKernel(int *_src, int offset, int stride) {
	extern __shared__ int partialSum[];
	unsigned int t = threadIdx.x;

	partialSum[t] = _src[offset+(blockIdx.x*blockDim.x+t+1)*stride-1];
//	printf("%d %d\n", t, partialSum[t]);

	for (unsigned int s = 1; s <= THREADS/2; s <<= 1) {
		__syncthreads();
		if ((t+1)%(s*2) == 0) {
			partialSum[t] = partialSum[t] + partialSum[t-s];
//			printf("stride:%d index:%d value:%d + %d = %d\n", s, t, t, t-s, partialSum[t]);
		}
	}
	__syncthreads();
	_src[offset+(t+1)*stride-1] = partialSum[t];
}

__global__ void prefixKernel(int *_src, int offset, int stride) {
	extern __shared__ int partialSum[];
	unsigned int t = threadIdx.x;
...
//	printf("%d %d\n", t, partialSum[t]);
	...
	for (unsigned int s = THREADS; s > 1; s >>= 1) {
		__syncthreads();
		if ((t+1) == s/2) {
...
int main(int argc, char* argv[])
{
	...
	cudaMemcpy2DToArray(gpuTex, 0, 0, img4, w * 4 * sizeof(float), 
	...
	cudaMemcpy(gpuFeatureImg, cpuGpu, nPix * sizeof(int),
	...
	else
	{
		...
		for (int i=0; i<h;i++){
			for (int j=0; j<w;j++)
				printf("%d ", cpuGpu[i*w+j]);
			printf("\n");
		}

		for (int i=0; i < h;i++){
			reduceKernel<<<blockGridReduce, threadBlock, w*sizeof(int)>>>(gpuFeatureImg, w*i, 1);
		}
		cudaMemcpy(cpuGpu, gpuFeatureImg, w*h * sizeof(int),
								cudaMemcpyDeviceToHost);
		printf("Reduce row-wize:\n");
		for (int i=0; i<h;i++){
			for (int j=0; j<w;j++)
				printf("%d ", cpuGpu[i*w+j]);
			printf("\n");
		}
		reduceKernel<<<blockGridReduce, threadBlock, w*sizeof(int)>>>(gpuFeatureImg, 0, 8);
		cudaMemcpy(cpuGpu, gpuFeatureImg, w*h * sizeof(int),
								cudaMemcpyDeviceToHost);
		...
		for (int i=0; i < h;i++){
			prefixKernel<<<blockGridReduce, threadBlock, w*sizeof(int)>>>(gpuFeatureImg, w*i, 1);
		}
		cudaMemcpy(cpuGpu, gpuFeatureImg, w*h * sizeof(int),
						cudaMemcpyDeviceToHost);

		printf("Prefix row-wize:\n");
		for (int i=0; i<h;i++){
			for (int j=0; j<w;j++)
				printf("%d ", cpuGpu[i*w+j]);
			printf("\n");
