...
__global__ void dL_dI_kernel(
    ...
    // TODO your kernel for dL_dI here
		...
		const int batch_size = dL_dO_padded.size(0);
		const int input_channels_size = dL_dO_padded.size(1);
		const int height = dL_dO_padded.size(2);
		const int width = dL_dO_padded.size(3);
		...
		if (output_x < width && output_y < height) {
			...
			for (int outc = 0; outc < output_channel_size; outc++) {
				scalar_t result = 0.0f;
				for (int inc = 0; inc < input_channels_size; inc++)
					result += dL_dO_padded[batch_index][outc][output_y+1][output_x+1] * w_c[inc][outc] +
							  (dL_dO_padded[batch_index][inc][output_y][output_x] + dL_dO_padded[batch_index][inc][output_y+1][output_x] +
							   dL_dO_padded[batch_index][inc][output_y+2][output_x] + dL_dO_padded[batch_index][inc][output_y+2][output_x+1] +
							   dL_dO_padded[batch_index][inc][output_y+2][output_x+2] + dL_dO_padded[batch_index][inc][output_y+1][output_x+2] +
							   dL_dO_padded[batch_index][inc][output_y][output_x+2] + dL_dO_padded[batch_index][inc][output_y][output_x+1]) *
							   w_s[inc][outc];

				dL_dI[batch_index][outc][output_y][output_x] = result;
			}
		}
}
...
std::vector<torch::Tensor> center_surround_convolution_backward (
	...
	// XXX: Use this padded version of dL_dO to compute dL_dI
		...
		const dim3 block_dim(8, 8, 1);
		const dim3 grid_dim((I_W+7)/8, (I_H+7)/8, O_C);
		...
		AT_DISPATCH_FLOATING_TYPES(
			  I.type(), "dL_dw_kernel", ([&] {
				dL_dw_kernel<scalar_t><<<grid_dim, block_dim>>>(
					dL_dO.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
					I.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
					O_dL_dw_c.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
					O_dL_dw_s.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>());
			  }));

		const dim3 grid_dim1((I_W+7)/8, (I_H+7)/8, I_B);

		AT_DISPATCH_FLOATING_TYPES(
			  I.type(), "dL_dI_kernel", ([&] {
				dL_dI_kernel<scalar_t><<<grid_dim1, block_dim>>>(
					dL_dO_padded.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>(),
					w_c.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
					w_s.packed_accessor32<scalar_t, 2, torch::RestrictPtrTraits>(),
					O_dL_dI.packed_accessor32<scalar_t, 4, torch::RestrictPtrTraits>());
			  }));
