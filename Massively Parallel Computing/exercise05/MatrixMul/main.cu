...
#define N 8 // Must be a multiple of THREADS_PER_BLOCK
#define THREADS_PER_BLOCK 8 // per axis -> block has this value squared threads.
		...
		const float* b, const int n)
{
	...
	int aBegin = by * THREADS_PER_BLOCK * n;
	int aEnd = aBegin + n - 1;
	int bBegin = bx * THREADS_PER_BLOCK;

	int aStep = THREADS_PER_BLOCK;
	int bStep = THREADS_PER_BLOCK * n;
...
//		printf("a: %d b: %d\n", aSub[ty][tx], bSub[ty][tx]);
	...
	int c = n * THREADS_PER_BLOCK * by + THREADS_PER_BLOCK * bx;
