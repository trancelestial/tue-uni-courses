	...
	for (int j = 0; j < _h; ++j)
	{
		for (int i = 0; i < _w; ++i)
		{
			...
			// !!! missing !!!
			 ...
			 res.x+= tex2D(texImg1, (float)i/_w,(float)j/_h).x * tex2D(texImg2, (float)(i+offsX)/_w, (float)(j+offsY)/_h).x;
			 res.y+= tex2D(texImg1, (float)i/_w,(float)j/_h).y * tex2D(texImg2, (float)(i+offsX)/_w, (float)(j+offsY)/_h).y;
			 res.z+= tex2D(texImg1, (float)i/_w,(float)j/_h).z * tex2D(texImg2, (float)(i+offsX)/_w, (float)(j+offsY)/_h).z;
...
int main(int argc, char* argv[])
{
	...
	cudaArray* gpuTex1;
	cudaArray* gpuTex2;
	cudaChannelFormatDesc floatTex1 = cudaCreateChannelDesc<float4>();
	cudaChannelFormatDesc floatTex2 = cudaCreateChannelDesc<float4>();
	cudaMallocArray( &gpuTex1, &floatTex1, w, h);
	cudaMallocArray( &gpuTex2, &floatTex2, w, h);

	cudaMemcpyToArray(gpuTex1, 0, 0, img3, w*h*sizeof(float4), cudaMemcpyHostToDevice);
	cudaMemcpyToArray(gpuTex2, 0, 0, img4, w*h*sizeof(float4), cudaMemcpyHostToDevice);

//	cudaBindTextureToArray( texImg, img, floatTex1) );
	// !!! missing !!!
	cudaBindTextureToArray(texImg1, gpuTex1, floatTex1);
	cudaBindTextureToArray(texImg2, gpuTex2, floatTex2);
