...
//			for (unsigned int pass = 0; pass < 2; pass++) {
//				// stream 1
//				cudaMemcpyAsync(gpuOperator2[pass], cpuOperator2[pass],
//														dim / 2 * sizeof(float), cudaMemcpyHostToDevice, stream[0]);
//
//				dotProdKernel<<<blockGrid, threadBlock, 0, stream[0]>>>(gpuResult[pass],
//														gpuOperator1[pass], gpuOperator2[pass], dim);
//				// stream 2
//				cudaMemcpyAsync(gpuOperator2[pass] + dim / 2 * sizeof(float), cpuOperator2[pass] + dim / 2 * sizeof(float),
//													dim / 2 * sizeof(float), cudaMemcpyHostToDevice, stream[1]);
//
//				dotProdKernel<<<blockGrid, threadBlock, 0, stream[1]>>>(gpuResult[pass],
//														gpuOperator1[pass], gpuOperator2[pass], dim/2);
//
//				cudaMemcpyAsync(cpuResult[pass], gpuResult[pass],
//										MAX_BLOCKS * MAX_THREADS * sizeof(float)/2,
...
//				int count = 0;
//				for (int i = 0; i < MAX_BLOCKS * MAX_THREADS; ++i){
//					if (cpuResult[pass][i] != 0)
//						printf("count: %d - %f\n", count++, cpuResult[pass][i]);
//					finalDotProduct += cpuResult[pass][i];
//				}
//				printf("Iteration %d, pass %d: %f\n", iter, pass,
//										finalDotProduct);
//			}
			cudaMemcpyAsync(gpuOperator2[0], cpuOperator2[0],
													dim * sizeof(float), cudaMemcpyHostToDevice, stream[0]);

			dotProdKernel<<<blockGrid, threadBlock, 0, stream[0]>>>(gpuResult[0],
													gpuOperator1[0], gpuOperator2[0], dim);
			// stream 2
			cudaMemcpyAsync(gpuOperator2[1], cpuOperator2[1],
												dim * sizeof(float), cudaMemcpyHostToDevice, stream[1]);

			dotProdKernel<<<blockGrid, threadBlock, 0, stream[1]>>>(gpuResult[1],
													gpuOperator1[1], gpuOperator2[1], dim);

			cudaMemcpyAsync(cpuResult[0], gpuResult[0],
									MAX_BLOCKS * MAX_THREADS * sizeof(float),
									cudaMemcpyDeviceToHost, stream[0]);

			cudaMemcpyAsync(cpuResult[1], gpuResult[1],
									MAX_BLOCKS * MAX_THREADS * sizeof(float),
									cudaMemcpyDeviceToHost, stream[1]);
